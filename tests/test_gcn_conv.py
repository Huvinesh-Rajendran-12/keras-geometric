...
try:
    ...
    raise ImportError(
        f"Could not import refactored GCNConv layer from package 'keras_geometric': {e}"
    ) from e
    ...
    class PyGGCNConv:
        ...
        def __call__(self, *args, **kwargs):
            ...
            raise NotImplementedError(
                "PyGGCNConv dummy is not callable. PyTorch/PyG is not available."
            )
...
class TestGCNConvComprehensive(unittest.TestCase):  # Renamed class
    def setUp(self):
        ...
        self.edge_index_np = np.array(
            [[0, 1, 1, 2, 3, 4, 0, 3], [1, 0, 2, 1, 4, 3, 2, 2]], dtype=np.int64
        )
    ...
    def test_refactored_call_shapes(self):
        ...
        for use_bias, normalize, add_loops in test_params:
            with self.subTest(
                use_bias=use_bias, normalize=normalize, add_loops=add_loops
            ):
                gcn = GCNConv(
                    output_dim=self.output_dim,
                    use_bias=use_bias,
                ...
                self.assertEqual(
                    output_shape,
                    expected_shape,
                    f"Shape mismatch for bias={use_bias}, norm={normalize}, loops={add_loops}",
                )
    ...
    def test_numerical_comparison_with_pyg(self):
        ...
        for use_bias, normalize, add_loops in test_params:
            ...
            with self.subTest(msg=subtest_msg):
                ...
                if "lin.weight" in pyg_params:
                    pyg_weight_param_name = "lin.weight"
                elif "weight" in pyg_params:
                    pyg_weight_param_name = "weight"
                else:
                    subtest_msg = (
                        f"bias={use_bias}, norm={normalize}, loops={add_loops}"
                    )
                    self.fail(
                        f"PyG layer missing weight parameter ('lin.weight' or 'weight') for {subtest_msg}"
                    )

                pyg_bias_param_name = None
                if use_bias:
                    if "bias" in pyg_params:
                        pyg_bias_param_name = "bias"
                    elif "lin.bias" in pyg_params:
                        pyg_bias_param_name = "lin.bias"
                    else:
                        self.fail(
                            f"PyG layer missing bias parameter ('bias' or 'lin.bias') for {subtest_msg}"
                        )
                else:
                    self.assertTrue(
                        "bias" not in pyg_params or pyg_params.get("bias") is None,
                        f"PyG layer has bias when use_bias=False for {subtest_msg}",
                    )
                    self.assertTrue(
                        "lin.bias" not in pyg_params
                        or pyg_params.get("lin.bias") is None,
                        f"PyG layer has lin.bias when use_bias=False for {subtest_msg}",
                    )
                # PyG parameter names already printed above

                print(
                    f"Syncing weights (PyG weight='{pyg_weight_param_name}', bias='{pyg_bias_param_name if use_bias else 'None'}')..."
                )
                if use_bias:
                    if len(keras_weights) != 2:
                        self.fail(
                            f"Expected 2 weights but got {len(keras_weights)} for {subtest_msg}"
                        )
                    k_kernel, k_bias = keras_weights[0], keras_weights[1]
                    pyg_params[pyg_weight_param_name].data.copy_(
                        torch.tensor(k_kernel.T)
                    )  # Use Transpose
                    pyg_params[pyg_bias_param_name].data.copy_(torch.tensor(k_bias))
                    synced_kernel_t = (
                        pyg_params[pyg_weight_param_name].data.cpu().numpy().T
                    )
                    ...
                    np.testing.assert_allclose(
                        ...
                        rtol=0,
                        atol=0,
                        err_msg="Kernel sync failed",
                    )
                    np.testing.assert_allclose(
                        k_bias, synced_bias, rtol=0, atol=0, err_msg="Bias sync failed"
                    )
                else:  # No bias
                    if len(keras_weights) != 1:
                        self.fail(
                            f"Expected 1 weight but got {len(keras_weights)} for {subtest_msg}"
                        )
                    ...
                    np.testing.assert_allclose(
                        ...
                        rtol=0,
                        atol=0,
                        err_msg="Kernel sync failed (no bias)",
                    )

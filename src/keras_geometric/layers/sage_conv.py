...
class SAGEConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        base_aggregator = (
            aggregator if aggregator in ["mean", "max", "sum"] else "max"
        )  # Use max as base for pooling
        ...
        self.pool_activation = activations.get(
            pool_activation
        )  # Get pool activation object
    ...
    def build(
        self, input_shape: tuple[tuple[Optional[int], int], tuple[int, Optional[int]]]
    ) -> None:
    ...
    def propagate(self, inputs: tuple[Any, Any]) -> "keras.KerasTensor":
        """
        ...
        if self.aggregator in ["mean", "max", "sum"]:
            ...
            aggregated_neighbors = super().propagate(
                inputs=(x, edge_index)
            )  # Shape: [N, F_in]

        elif self.aggregator == "pooling":
            if self.pool_mlp is None:
                raise RuntimeError("Pool MLP not built.")
            ...
            aggr = ops.segment_max(
                data=h_neighbors_mlp, segment_ids=target_idx, num_segments=num_nodes
            )
            # Replace potential -inf (for nodes with no neighbors) with zeros
            aggregated_neighbors = ops.where(
                ops.isinf(aggr), ops.zeros_like(aggr), aggr
            )  # Shape: [N, F_in]
        else:
            # Should be unreachable due to __init__ validation
            raise ValueError(
                f"Internal error: Unsupported aggregator '{self.aggregator}'"
            )
        return aggregated_neighbors

    def call(self, inputs: tuple[Any, Any]) -> "keras.KerasTensor":
        ...
        if self.lin_neigh is None:
            raise RuntimeError("lin_neigh layer not built.")
        h_neigh = self.lin_neigh(
            aggregated
        )  # Shape: [N, output_dim] (Bias is NOT added here)

        # --- 3. Transform self features and combine (if root_weight=True) ---
        if self.root_weight:
            if self.lin_self is None:
                raise RuntimeError("lin_self layer not built.")
        ...
        config.update(
            {
                ...
                "pool_activation": activations.serialize(
                    getattr(self, "pool_activation", None)
                ),
            }
        )
